...
def get_sensor_readout_coordinates(robot_coord, laser, laser_angles, orientation):
    """
    Requests the current laser properties from
    the MRDS server and parses it into a dict
    """
...
def get_bresenham_lines(robot_coord, sensor_readout_coordinates):
    """
    Yield integer coordinates on the line from (x0, y0) to (x1, y1).
    Input coordinates should be integers.
    The result will contain both the start and the end point.
    ...
    for x in range(0, len(sensor_readout_coordinates)):
        ...
        bresenhamLine = list(
            bresenham(robot_coord[0], robot_coord[1], sensor_readout_coordinates[x][0], sensor_readout_coordinates[x][1]))
        # print(bresenhamLine)
        # print("\n")
